...
# BUG FIX (v7-1): dropped "RewriteBase /" — this file loads as server/vhost
# config (conf-enabled/*.conf, outside any <Directory>), and RewriteBase is
# valid ONLY in per-directory context (.htaccess or <Directory>). Leaving it
# here made Apache fail its config test on EVERY start with:
#   "RewriteBase: only valid in per-directory config files"
# — a fatal error, so the wordpress container crash-looped forever and never
# actually served anything on port 80 (only Podman's own port-forward stayed
# up behind it).
# BUG FIX (v7-2): the two rules above rewrote the bare slug to "/wp-admin"
# — a DIRECTORY, not a file. WordPress's own core rule in .htaccess always
# targets a literal file (/index.php) for exactly this reason: Apache's
# directory/trailing-slash resolution on an internally-rewritten directory
# target is fragile, and in testing it silently swallowed the rewrite —
# visiting /${WP_ADMIN_SLUG} fell through to WordPress's normal front-end
# routing and rendered its own 404 template instead of reaching wp-admin at
# all. Targeting literal files (index.php, wp-login.php, and whatever file
# a wp-admin sub-path already names, e.g. post.php, admin-ajax.php) removes
# that ambiguity entirely — every RewriteRule target below is a real file.
...
CROWDSEC_IMAGE="docker.io/crowdsecurity/crowdsec:v1.7.6"
...
sleep 3  # Brief wait for entrypoint to finish initial file copy
PRUN exec wordpress chown -R www-data:www-data /var/www/html/wp-content \
  && ok "wp-content/ ownership → www-data:www-data (uploads now writable)" \
  || warn "chown failed — uploads may not be writable; fix: PRUN exec wordpress chown -R www-data:www-data /var/www/html/wp-content"
...
#
# DESIGN NOTE — why this differs from a typical "podman exec && compile" guide:
#   Compiling mod_maxminddb via `podman exec` into a RUNNING container writes
#   to that container's ephemeral writable layer. The next time the container
#   is recreated (an update, an OpenRC restart after `podman rm`, a host
#   reboot that re-runs the recreate-if-absent logic), that compiled module
#   is GONE — silently, until someone notices GeoIP stopped working.
#   Instead we build a small custom image with `podman build` (multi-stage:
#   one stage compiles, the final stage is the original image plus only the
#   compiled .so — no leftover build-essential/gcc in the production image).
#   This new image becomes the WordPress image for the rest of this install
#   AND for every future update/recreate, so GeoIP survives indefinitely
#   without any persistence hacks.
#   The GeoLite2 database itself is architecture-agnostic binary data, so it
#   does not need to live inside the image — it's fetched directly via curl
#   on the Alpine host (documented MaxMind permalink API) and bind-mounted
#   in, refreshed weekly via host cron. This avoids depending on whether
#   Alpine packages `geoipupdate` at all.
# ════════════════════════════════════════════════════════════════════════════
if [ "${GEOIP_ENABLED:-0}" = "1" ] && [ -n "${MAXMIND_ACCOUNT_ID}" ] && [ -n "${MAXMIND_LICENSE_KEY}" ]; then
  ...
  GEOIP_IMG_TAG="localhost/wordpress-geoip:$(echo "${WP_IMAGE}" | sed 's|.*:||')"
  mkdir -p /home/wpuser/wp/geoip-build /home/wpuser/wp/geoip-db /home/wpuser/wp/apache-mods

  # Resolve the latest mod_maxminddb release tarball URL via the GitHub API.
  # Falls back to a known-good pinned version if the API call fails (rate
  # limit, no network at this exact moment, etc.) so the build is not 100%
  ...
  if [ -z "$MMDB_ASSET_URL" ]; then
    ...
    warn "GitHub API lookup failed — falling back to pinned mod_maxminddb 1.2.0"
  else
    ok "Latest mod_maxminddb release: $(basename "$MMDB_ASSET_URL")"
  fi

  # Multi-stage Containerfile:
  #   Stage "builder": full Debian build toolchain, compiles mod_maxminddb.so
  #   Stage final: starts fresh FROM the same pinned WordPress image, copies
  #     in ONLY the compiled .so — zero build tools in the production image.
  cat > /home/wpuser/wp/geoip-build/Containerfile << CONTAINERFILE
FROM ${WP_IMAGE} AS builder
...
CONTAINERFILE
  ...
  ok "Containerfile written — building (compiles inside container, ~1-2 min)…"
  if podman build -t "${GEOIP_IMG_TAG}" -f /home/wpuser/wp/geoip-build/Containerfile /home/wpuser/wp/geoip-build >> "$LOG" 2>&1; then
    ok "Custom image built: ${GEOIP_IMG_TAG} (mod_maxminddb compiled in, build tools left behind)"

    # LoadModule line — static, never changes, so write once here.
...
MMLOAD
    ...
    # Fetch GeoLite2-Country.mmdb directly via MaxMind's documented permalink
    # download API (HTTP Basic Auth with Account ID + License Key). This is
    # plain HTTPS binary data — no geoipupdate binary dependency needed.
    ts "Fetching GeoLite2-Country database"
    if curl -fsSL -u "${MAXMIND_ACCOUNT_ID}:${MAXMIND_LICENSE_KEY}" \
         'https://download.maxmind.com/geoip/databases/GeoLite2-Country/download?suffix=tar.gz' \
         -o /tmp/geolite2-country.tar.gz 2>>"$LOG"; then
      mkdir -p /tmp/geolite-extract
      tar xzf /tmp/geolite2-country.tar.gz -C /tmp/geolite-extract --strip-components=1 2>/dev/null
      ...
      if [ -s /home/wpuser/wp/geoip-db/GeoLite2-Country.mmdb ]; then
        ...
        ok "GeoLite2-Country.mmdb downloaded ($(du -h /home/wpuser/wp/geoip-db/GeoLite2-Country.mmdb | cut -f1))"
        GEOIP_DB_OK=1
      else
        warn "Download succeeded but no .mmdb extracted — check MaxMind credentials"
        GEOIP_DB_OK=0
      fi
    else
      warn "GeoLite2 download failed — verify MAXMIND_ACCOUNT_ID / MAXMIND_LICENSE_KEY are correct"
      GEOIP_DB_OK=0
    fi

    if [ "${GEOIP_DB_OK:-0}" = "1" ]; then
      # Build the Apache directive block. SetEnvIf + RequireAll/Require env
      # is used (rather than ap_expr "in" syntax) because it is the
      # documented, widely-verified-working pattern for mod_maxminddb,
      # using only mod_setenvif + mod_authz_core which are both already
      # present in the stock Debian Apache build.
...
# GeoIP country filtering — generated by create-wordpress-vm.sh
...
GEOIPCONF
      ...
      ok "geoip.conf written (${GEOIP_MODE}: ${GEOIP_WHITELIST:-$GEOIP_BLOCKLIST})"

      # Swap WP_IMAGE to the custom-built tag for the REST of this installer
      # (OpenRC service heredocs below substitute ${WP_IMAGE} at write time,
      # so this reassignment flows through automatically) and recreate the
      # running container with the GeoIP module + config now mounted in.
      WP_IMAGE="${GEOIP_IMG_TAG}"
      ts "Restarting WordPress with GeoIP module active"
      if [ "${ROOTLESS_MODE:-0}" = "1" ]; then
        # Regenerate run-wordpress.sh cleanly (not a sed-patch — the file
        # already has its image tag resolved to a literal string from
        # generation time, so pattern-matching the variable name would not
        # match anything) so it remains the single source of truth that
        # update.sh will also reference for future rootless WordPress updates.
        cat > /home/wpuser/wp/run-wordpress.sh << RUNWPGEO
#!/bin/sh
# Generated by create-wordpress-vm.sh — rootless WordPress launcher (GeoIP active).
if podman container exists wordpress 2>/dev/null; then
  podman start wordpress
else
  podman run -d --name wordpress --network wp-net --ip 10.89.1.3 -p 8080:80 --restart always \\
...
RUNWPGEO
        chmod 750 /home/wpuser/wp/run-wordpress.sh
        chown wpuser:wpuser /home/wpuser/wp/run-wordpress.sh
        PRUN rm -f wordpress >/dev/null 2>&1
        su -s /bin/sh wpuser -c '/home/wpuser/wp/run-wordpress.sh'
      else
        podman rm -f wordpress >/dev/null 2>&1
        podman run -d \
          --name    wordpress \
          --network wp-net \
          --ip      10.89.1.3 \
      ...
      for i in $(seq 1 12); do
        wget -qO- "http://127.0.0.1:${WEB_CHECK_PORT}/" >/dev/null 2>&1 && { ok "WordPress responding with GeoIP active"; break; }
        sleep 5
      done

      # Weekly database refresh — same direct-curl approach, host-side cron.
      cat >> /etc/crontabs/root << GEOCRON
...
GEOCRON
      ok "Weekly GeoIP database refresh scheduled (Wed 06:00 UTC)"
    fi
  else
    warn "GeoIP image build failed — check ${LOG} for the podman build output. Continuing without GeoIP."
  fi
...
# Usage: update.sh [check|os|wp [VER]|db [VER]|crowdsec [VER]|all]
# =============================================================================
set -e
...
RUNNING_WP=$(PRUN inspect wordpress --format "{{.Config.Image}}" 2>/dev/null \
  | sed "s|${WP_REGISTRY}:||" || echo "not running")
RUNNING_DB=$(PRUN inspect mariadb   --format "{{.Config.Image}}" 2>/dev/null \
  | sed "s|${DB_REGISTRY}:||" || echo "not running")
RUNNING_CS=$(PRUN inspect crowdsec  --format "{{.Config.Image}}" 2>/dev/null \
  | sed "s|${CS_REGISTRY}:||" || echo "not running")
...
do_wp_update() {
  ...
  if PRUN run -d --name wordpress --network wp-net --ip 10.89.1.3 "-p ${WP_PORT}:80" --restart always \
    ...
    if [ "$HEALTHY" = "1" ]; then
      ...
      sed -i "s|WP_IMAGE=.*|WP_IMAGE=\"${WP_REGISTRY}:${target_ver}\"|" /etc/init.d/wp-container 2>/dev/null || true
      # Re-apply uploads ownership after update (entrypoint re-creates dirs as root)
      sleep 3
      PRUN exec wordpress chown -R www-data:www-data /var/www/html/wp-content >/dev/null 2>&1 || true
      echo "✔  WordPress updated to ${target_ver}"
    else
...
do_db_update() {
  ...
  if PRUN run -d --name mariadb --network wp-net --restart always \
    ...
    if [ "$DB_READY" = "1" ]; then
      ...
      sed -i "s|DB_IMAGE=.*|DB_IMAGE=\"${DB_REGISTRY}:${target_ver}\"|" /etc/init.d/mariadb-container 2>/dev/null || true
...
case "${1:-check}" in
  ...
  check|"")
    ...
    echo "╚══════════════════════════════════════════════════════════╝"
    echo ""
    do_os_update; do_wp_update; do_db_update; do_cs_update; show_status ;;
  *) echo "Usage: update.sh [check|os|wp [VER]|db [VER]|crowdsec [VER]|all]"; exit 1 ;;
esac
UPDSCRIPT
chmod +x /usr/local/bin/update.sh
ok "update.sh installed (wp / db / crowdsec / os / all)"
...
msg_ok "Installer vars injected (slug=${WP_ADMIN_SLUG:-default}, cs-enroll=${CROWDSEC_ENROLL_KEY:+provided}, net=${NET_MODE}, geoip=${GEOIP_ENABLED:-0}, rootless=${ROOTLESS_MODE:-0})"
